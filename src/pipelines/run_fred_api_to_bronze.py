...
from src.etl.schema.fred_metadata_schema import fred_metadata_schema, METADATA_REQUIRED_COLUMNS
...
def run():
    ...
    config = load_config()
    ppl_name = "run_fred_api_to_bronze"
    layer = "bronze"
    source_table = None
    target_table = config["tables"]["bronze_fred_macro_indicators"]
    ...
    #TODO: Move iteration to extractor
        ...
        config_params = json.dumps({
            "mode": "BOOTSTRAP" if not has_data else "REFRESH",
            "observation_start": observation_start,
            "series_count": len(series_ids),
            "success_count": success_count,
            "failure_count": failure_count,
            "fact_rows": fact_df.count(),
            "metadata_rows": dim_df.count()
        })
